...
public class DeviceConnectionPool {
    private final LruHashMap<String, BreoDevice> DEVICE_MIRROR_MAP;

    public DeviceConnectionPool() {
        DEVICE_MIRROR_MAP = new LruHashMap<>(1);
    }

    public DeviceConnectionPool(int deviceMirrorSize) {
    ...
    public synchronized void addDeviceMirror(BluetoothLeDevice bluetoothLeDevice) {
        ...
        if (!DEVICE_MIRROR_MAP.containsKey(key)) {
            DEVICE_MIRROR_MAP.put(key, new BreoDevice(bluetoothLeDevice));
        }
    }
     ...
     * @param deviceMirror
     */
    public synchronized void addBreoDevice(BreoDevice deviceMirror) {
     ...
     * @param deviceMirror
     */
    public synchronized void removeDeviceMirror(BreoDevice deviceMirror) {
        if (deviceMirror == null) {
            return;
        }
        if (DEVICE_MIRROR_MAP.containsKey(deviceMirror.getUniqueSymbol())) {
     ...
     * @return
     */
    public synchronized boolean isContainDevice(BreoDevice deviceMirror) {
    ...
    public synchronized ConnectState getConnectState(BluetoothLeDevice bluetoothLeDevice) {
        BreoDevice deviceMirror = getDeviceMirror(bluetoothLeDevice);
        if (deviceMirror != null) {
     ...
     * @return
     */
    public synchronized BreoDevice getDeviceMirror(BluetoothLeDevice bluetoothLeDevice) {
        if (bluetoothLeDevice != null) {
            String key = bluetoothLeDevice.getAddress() + bluetoothLeDevice.getName();
            if (DEVICE_MIRROR_MAP.containsKey(key)) {
    ...
    public synchronized void disconnect() {
        for (Map.Entry<String, BreoDevice> stringDeviceMirrorEntry : DEVICE_MIRROR_MAP.entrySet()) {
     ...
     * @return
     */
    public Map<String, BreoDevice> getDeviceMirrorMap() {
     ...
     * @return
     */
    public synchronized List<BreoDevice> getDeviceMirrorList() {
        final List<BreoDevice> deviceMirrors = new ArrayList<>(DEVICE_MIRROR_MAP.values());
        Collections.sort(deviceMirrors, new Comparator<BreoDevice>() {
            @Override
            public int compare(final BreoDevice lhs, final BreoDevice rhs) {
    ...
    public synchronized List<BluetoothLeDevice> getDeviceList() {
        ...
        for (BreoDevice deviceMirror : getDeviceMirrorList()) {
            if (deviceMirror != null) {
