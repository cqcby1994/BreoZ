...
public class HexUtil {
    ...
    public static String toHex16(byte b) {
        return "0x" + M_CHARS[(b & 0xFF) >> 4] + M_CHARS[b & 0x0F];
    }
    public static String toHex16Plus(byte b) {
        return "" + M_CHARS[(b & 0xFF) >> 4] + M_CHARS[b & 0x0F];
    }

    public static String toHexString(byte[] data) {
        StringBuilder sb = new StringBuilder();
        for (byte aData : data) {
            sb.append(M_CHARS[(aData & 0xFF) >> 4]);
            sb.append(M_CHARS[aData & 0x0F]);
            sb.append(" ");
        }
